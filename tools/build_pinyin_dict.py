...
def place_entries(directory: str, known: set):
    """
    Yields (word, [syllables], count) from every `*.tsv` in [directory].

    The files are committed, generated by `tools/build_taipei_places.py`; each line is
    `name<TAB>reading<TAB>kind<TAB>count`, the count on phrase.occ's scale. Validated here rather
    than trusted, because a reading the syllable table does not know would make the entry
    unreachable without anything failing. [known] is every syllable the upstream words already
    use: a place name may not introduce a new one, or `SyllableTable.kt` (generated from the
    shipped dictionary) would silently stop matching it.
    """
    for path in sorted(os.listdir(directory)) if os.path.isdir(directory) else ():
        ...
        with open(os.path.join(directory, path), encoding="utf-8") as f:
            for number, line in enumerate(f, 1):
                if line.startswith("#") or not line.strip():
                    continue
                word, reading, _kind, count = line.rstrip("\n").split("\t")
                syls = reading.split()
                if len(syls) != len(word) or len(word) > MAX_WORD_LEN:
                    raise SystemExit(f"{path}:{number}: {word} does not align with {reading}")
                unknown = [s for s in syls if s not in known]
                if unknown:
                    raise SystemExit(f"{path}:{number}: {word} has unknown syllables {unknown}")
                # Deliberately no Simplified-character check, unlike [mcbopomofo_entries]: official
                # Taiwan names use characters OpenCC classes as Simplified -- the Metro signs
                # 頭前庄 with 庄, which BPMFBase tags big5. Government registers are the
                # authority on how these names are written.
                yield word, syls, float(count)
...
def build(work: str, out_path: str, max_entries: int, places_dir: str) -> None:
    log("sources")
    ...
    # Entries the global cap may not drop; see the prune step.
    protected: set = set()
    ...
    # --- place names -----------------------------------------------------------------------
    # Names no upstream dictionary carries -- 忠孝新生, 南京復興, most of Taipei's 里 and
    # streets -- generated from government open data by tools/build_taipei_places.py. See that
    # file for where the readings come from and why they can be trusted.
    # The weight is the name's own frequency: its count in Taiwan text (PTT and Common Crawl),
    # already rescaled by build_taipei_places.py onto phrase.occ's scale, then onto the mainland
    # scale exactly as every McBopomofo word is. So a place competes with 中小 or 藍芽 on the
    # same footing as any other word, and a name nobody writes ranks like one. The floor is one
    # occurrence, the same floor McBopomofo's unseen words get. A name McBopomofo also counted
    # keeps the larger of the two measurements, which is what [offer] does for every source.
    n_places = 0
    unseen = 0
    known = {s for (_word, syls) in words for s in syls.split()}
    for word, syls, count in place_entries(places_dir, known):
        key = " ".join(syls)
        if count < 1:
            unseen += 1
        offer((word, key), int(max(count, 1.0) * scale), REGION_TW)
        protected.add((word, key))
        n_places += 1
    log(f"  place names: {n_places} entries, {unseen} at the one-occurrence floor")
    ...
    for syls, items in by_key.items():
        ...
        for rank, (_w, word, weight) in enumerate(items):
            # Place names survive the per-key cap for the same reason they survive the global
            # one below: they rank by their weight, but they must exist to be typed at all.
            if rank < MAX_PER_KEY or (word, syls) in protected:
    ...
    if max_entries and len(kept) > max_entries:
        # The cut line falls inside a tie: thousands of entries weigh exactly 100 (ext.dict's
        # unweighted floor), and the stable sort keeps whichever come first. So every word added
        # above the line evicts one of those floor entries and nothing heavier -- adding the
        # Taipei place names cost 2,174 of them (玻璃匠, 不得睡) and no weighted word.
        # Place names are exempt. Their weight is a frequency and ranks them honestly, but a
        # name nobody wrote in the corpus sits at the floor -- below this cut line -- and being
        # typeable at all is the reason they were added. Ranking is the weight's job; this cap
        # is only about asset size, and 2,500 entries do not move it.
        kept.sort(key=lambda t: ((t[1], t[0]) not in protected, -max(t[2])))
...
def main() -> None:
    ...
    ap.add_argument("--places", default=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                     "places"),
                    help="directory of place-name TSVs from build_taipei_places.py")
    args = ap.parse_args()
    build(args.work, args.out, args.max_entries, args.places)
